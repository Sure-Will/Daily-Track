...
- 通过日历界面按日期记录习惯完成情况，每个习惯支持「月视图」与「年视图」两种日历。
- 每个习惯卡片内嵌迷你月历，并根据窗口宽窄自适应布局。
...
- 自动刷新保持界面同步：外部打卡（如经 Hermes bridge 写入）约 30 秒内自动出现，跨午夜自动滚动到新日期，且刷新不会覆盖正在进行的本地打卡。
- 支持导入/导出本地 JSON 备份。
...
- 应用每约 30 秒轮询一次存储，因此外部写入（Hermes/Telegram）会自动显示。仅当数据确实发生变化时才重建界面，且本地保存进行中时轮询会让位，绝不丢掉你刚记录的打卡。
